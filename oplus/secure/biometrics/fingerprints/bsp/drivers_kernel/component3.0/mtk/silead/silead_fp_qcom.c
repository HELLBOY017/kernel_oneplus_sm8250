...
static void silfp_hw_poweroff(struct silfp_data *fp_dev)
{
	...
	if ((fp_dev->power_is_off == 0) && fp_dev->avdd_ldo
	...
	if ((fp_dev->power_is_off == 0) && fp_dev->vddio_ldo
...
static int silfp_parse_dts(struct silfp_data *fp_dev)
{
	...
	fp_dev->pin.active = pinctrl_lookup_state(fp_dev->pin.pinctrl,
...
#ifndef QSEE_V4
	/* Get sleep settings */
	fp_dev->pin.sleep = pinctrl_lookup_state(fp_dev->pin.pinctrl,
	...
	/* Get the QUP ID (#1-12) */
	ret = of_property_read_u32(fp_dev->spi->dev.of_node, "qcom,qup-id",
			&fp_dev->pin.qup_id);
