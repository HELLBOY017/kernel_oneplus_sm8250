...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	if (gf_dev->device_available == 0) {
		...
		(cmd == GF_IOC_CS_PULL_DOWN) ||
		(cmd == GF_IOC_RST_PULL_DOWN)) {
		pr_info("power cmd\n");
            }
           else {
    ...
    case GF_IOC_STOP_WAIT_INTERRUPT_EVENT:
        ...
        send_fingerprint_message(E_FP_HAL, 0, NULL, 0);
        break;
	case GF_IOC_CS_PULL_DOWN:
	    (void) gf_cs_pull_down(gf_dev);
	    pr_info("%s GF_IOC_CS_PULL_DOWN. \n", __func__);
            break;
	case GF_IOC_RST_PULL_DOWN:
	    (void) gf_rst_pull_down(gf_dev);
	    pr_info("%s GF_IOC_RST_PULL_DOWN. \n",__func__);
	    break;
	default:
