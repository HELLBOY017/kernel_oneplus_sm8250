...
int gf_parse_dts(struct gf_dev* gf_dev)
{
    ...
    gf_dev->pstate_cs_pull_down = NULL;
    gf_dev->pstate_rst_pull_down = NULL;
    ...
    if(gf_dev->is_optical == true) {
        ...
        gf_dev->pstate_cs_pull_down = pinctrl_lookup_state(gf_dev->pinctrl, "goodix_cs_pull_down");
        if (IS_ERR(gf_dev->pstate_cs_pull_down)) {
            dev_err(&pdev->dev, "Can't find cs_pull_down pinctrl state\n");
        } else {
            pr_info("find pstate_cs_pull_down ok.\n");
        }
        gf_dev->pstate_rst_pull_down = pinctrl_lookup_state(gf_dev->pinctrl, "goodix_rst_pull_down");
        if (IS_ERR(gf_dev->pstate_rst_pull_down)) {
        dev_err(&pdev->dev, "Can't find pstate_rst_pull_down pinctrl state\n");
        } else {
            pr_info("find pstate_rst_pull_down ok.\n");
        }
    }
	/*get reset resource*/
...
{
    int cs_pull_down_temp;
    if (gf_dev == NULL) {
        pr_info("Input buff is NULL. \n");
        return -1;
    }
    if (IS_ERR(gf_dev->pstate_cs_pull_down) == 0) {
        cs_pull_down_temp = pinctrl_select_state(gf_dev->pinctrl, gf_dev->pstate_cs_pull_down);
        if (cs_pull_down_temp) {
            pr_info("Can't find cs_pull_down pinctrl state\n");
        } else {
            pr_info("%s:select cs pstate pull down. \n", __func__);
        }
        pr_info("cs_pull_down_temp = %d\n", cs_pull_down_temp);
        return cs_pull_down_temp;
    }
    return 0;
}
int gf_rst_pull_down(struct gf_dev *gf_dev)
{
    int rst_pull_down_temp;
    if (gf_dev == NULL) {
    pr_info("Input buff is NULL. \n");
    return -1;
    }
    if (IS_ERR(gf_dev->pstate_rst_pull_down) == 0) {
    rst_pull_down_temp = pinctrl_select_state(gf_dev->pinctrl, gf_dev->pstate_rst_pull_down);
    if (rst_pull_down_temp) {
        pr_info("Can't find pstate_rst_pull_down pinctrl state\n");
    } else {
        pr_info("%s:select rst pstate pull down. \n", __func__);
    }
        pr_info("rst_pull_down_temp = %d\n", rst_pull_down_temp);
        return rst_pull_down_temp;
    }
    return 0;
}
